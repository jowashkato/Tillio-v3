              ...
              if (newValue != 0) {
                if (await Helper().checkConnectivity()) {
                  ...
                  await ContactPaymentApi()
                      ...
                      .then((value) {
                    if (value != null) {
                      var openingBalance = value['data'][0]['opening_balance'] ?? 0.0;
                      var openingBalancePaid = value['data'][0]['opening_balance_paid'] ?? 0.0;
                      var sellDue = value['data'][0]['sell_due'] ?? 0.0;
                      if (openingBalance is! double) {
                        openingBalance = double.tryParse(openingBalance.toString()) ?? 0.0;
                      }
                      if (openingBalancePaid is! double) {
                        openingBalancePaid = double.tryParse(openingBalancePaid.toString()) ?? 0.0;
                      }
                      if (sellDue is! double) {
                        sellDue = double.tryParse(sellDue.toString()) ?? 0.0;
                      }
                      var balance = (sellDue + openingBalance) - openingBalancePaid;
                      // print(balance);
                      due = balance.toString();
                      setState(() {
